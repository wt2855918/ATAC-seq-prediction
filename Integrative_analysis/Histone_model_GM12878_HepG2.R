...
myinf1 = "/lorax/chenglab/yanding/ATAC_seq_integration/Data/HepG2/Final_ID/Histone_integration_prediction.Rda"
load(myinf1)
...
#begin to plot the relative importance plot 
importance = fit[[6]]
com = intersect(row.names(importance),row.names(HepG2_info))
importance = importance[com,]
HepG2_info = HepG2_info[com,]
row.names(importance) = HepG2_info$Experiment.target 
...
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/MS_Figure/Figure_3/HepG2_histone_importance_bxplot.pdf"
pdf(myoutf1, width= 4.5, height= 3)
p <- position_dodge(0.1)
p1 <- plot_gene(data,tar) + scale_fill_manual(values=rep(c1,11))+guides(fill=FALSE) + scale_y_continuous(breaks= pretty(data$EDS,n=4))
p9 <- p1 + theme(axis.text.y=element_text(size= 7.25),axis.text.x=element_text(size= 7.25,angle=45),axis.title.y=element_text(size= 7.25),axis.title.x=element_blank())
p10 <- p9 +ggtitle("HepG2") + theme(aspect.ratio=1,panel.border = element_blank(),axis.line = element_line(colour="black"),axis.title=element_text(size=7.25),plot.title=element_text(size=7.25,hjust = 0.5,vjust=0.5))
...
#begin to plot the relative importance plot 
importance = fit[[6]]
com = intersect(row.names(importance),row.names(GM12878_info))
importance = importance[com,]
GM12878_info = GM12878_info[com,]
row.names(importance) = GM12878_info$Experiment.target 
rank = apply(importance,1,mean)
rank = rank[order(rank,decreasing=T)]
...
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/MS_Figure/Figure_3/GM12878_histone_importance_bxplot.pdf"
pdf(myoutf1, width= 4.5, height= 3)
p <- position_dodge(0.1)
p1 <- plot_gene(data,tar) + scale_fill_manual(values=rep(c1,11))+guides(fill=FALSE) + scale_y_continuous(breaks= pretty(data$EDS,n=4))
p9 <- p1 + theme(axis.text.y=element_text(size= 7.25),axis.text.x=element_text(size= 7.25,angle=45),axis.title.y=element_text(size= 7.25),axis.title.x=element_blank())
p10 <- p9 +ggtitle("GM12878") + theme(aspect.ratio=1,panel.border = element_blank(),axis.line = element_line(colour="black"),axis.title=element_text(size=7.25),plot.title=element_text(size=7.25,hjust = 0.5,vjust=0.5))
#p10 <- p10+ annotate("text",x=1,y=py[length(py)-2],label=paste0("p=",pval),size=2.5,)
p10
dev.off()
...
res = matrix(0,4000,11)
row.names(res) = row.names(HepG2_data)
colnames(res) = GM12878_info$target
res = as.data.frame(res)
...
myoutf = "/lorax/chenglab/yanding/ATAC_seq_integration/Data/GM12878/Integration/GM12878_Histone_signal_HepG2_validation_region.Rda"
...
myoutf = "/lorax/chenglab/yanding/ATAC_seq_integration/Data/HepG2/Integration/HepG2_Histone_signal_GM12878_validation_region.Rda"
save(res,file =myoutf)
...
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/MS_Figure/Figure_3/GM12878_Histone_Model_in_HepG2_AUC.pdf"
...
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/MS_Figure/Figure_3/HepG2_Histone_Model_in_GM12878_AUC.pdf"
pdf(myoutf1, width= 3, height= 3)
p <- position_dodge(0.1)
p1 <- ggplot(data=data, aes(x=target, y=V1, fill=c1)) + 
  geom_bar(stat="identity", color="black", position=position_dodge(),width=0.5) + scale_fill_manual(values=c(c1))+guides(fill=FALSE)+scale_y_continuous(expand = c(0,0),limits = c(0, 0.85)) + geom_text(aes(label=V1), vjust=1.6, color="white", size=3.5)
