...
tmpinf =  "/lorax/chenglab/yanding/ATAC_seq_integration/Data/GM12878/positive_region_annotation.txt"
...
myoutf = "/lorax/chenglab/yanding/ATAC_seq_integration/Data/GM12878/Final_ID/TF_prediction_promoter_distal.Rda"
...
colnames(res)[1:3] = c("x","y","z")
res$Annotation = factor(res$Annotation, levels =c("Promoter","Distal"))
x = as.numeric(res$x)
y = as.numeric(res$y)
z = as.numeric(res$z)
...
library(plot3D)
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/GM12878_TF_Proximal_vs_distal_association_3D.pdf"
pdf(myoutf1,width=7.5,height=5)
scatter3D(x, y, z, bty = "g", pch = 18, cex=0.1, theta = 20, phi = 20,
          col.var = as.integer(res$Annotation), 
          col = c("red", "blue"),
          pch = 18, ticktype = "detailed",
          colkey = list(at = c(1, 2), side = 1, 
...
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/GM12878_TF_Proximal_vs_distal_association.pdf"
...
myoutf = "/lorax/chenglab/yanding/ATAC_seq_integration/Data/GM12878/Final_ID/Histone_prediction_promoter_distal.Rda"
save.image(file=myoutf)
...
library(plot3D)
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/GM12878_Histone_Proximal_vs_distal_association_3D.pdf"
pdf(myoutf1,width=7.5,height=5)
scatter3D(x, y, z, bty = "g", pch = 18, cex=0.1, theta = 20, phi = 20,
          col.var = as.integer(res$Annotation), 
          col = c("red", "blue"),
          pch = 18, ticktype = "detailed",
          colkey = list(at = c(2, 1), side = 1, 
          addlines = TRUE, length = 0.5, width = 0.5))
dev.off()
...
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/GM12878_Histone_Proximal_vs_distal_association.pdf"
pdf(myoutf1, width= 3, height= 3)
p <- ggplot(res, aes(x=x,y=y,color=Annotation)) +   theme(legend.position="none")  +  geom_point(size=0.25)+scale_color_manual(values=c(c1,c2))
p1 <- p +  ggtitle("GM12878 \n (Proximal vs Distal)")
p2 <- p1 + theme(panel.grid.minor = element_line(colour="green", size=0.5),panel.grid.major = element_line(colour="green", size=0.5)) + theme_bw()
...
rm(list=ls())
myinf1 = "/lorax/chenglab/yanding/ATAC_seq_integration/Data/GM12878/Final_ID/TF_prediction_promoter_distal.Rda"
...
myoutf <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/ROC_Curve_TF_histone_self_prediction.pdf"
...
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/TF_histone_self_proximal_distal_prediction_barplot.pdf"
pdf(myoutf1, width= 3, height= 3)
...
myoutf <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/ROC_Curve_TF_histone_model_prediction.pdf"
pdf(myoutf, width= 5, height= 5)
par(pty="s")
plot(TF_xx1,TF_yy1,main="", xlab="False Positive Rate (1-Specificity)",ylab="True Positive Rate (Sensitivity)",
xlim=c(0,1),ylim=c(0,1),cex=0,cex.main=1.5,cex.lab=1,font.main=2) #xaxs="i",yaxs="i"
...
myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/TF_histone_other_Model_proximal_distal_prediction_barplot.pdf"
pdf(myoutf1, width= 3, height= 3)
p <- position_dodge(0.1)
p1 <- ggplot(data=data, aes(x=target, y=AUC, fill=color)) + 
