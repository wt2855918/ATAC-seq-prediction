...
info = info[com,]
res = res[com,]
res$Target = info$Experiment.target
info$AUC = res$AUC
...
colnames(mESC_info) = colnames(GM12878_info) = c("file_accession","target")

row.names(mESC_info) = mESC_info$target
row.names(GM12878_info) = GM12878_info$target

com = intersect(row.names(mESC_info),row.names(GM12878_info))
mESC_info = mESC_info[com,]
GM12878_info = GM12878_info[com,]

row.names(mESC_info) = mESC_info$file_accession
row.names(GM12878_info) = GM12878_info$file_accession

mESC_data = mESC_data[,row.names(mESC_info)]
GM12878_data = GM12878_data[,row.names(GM12878_info)]

colnames(mESC_data) = mESC_info$target
...
mESC_model = randomForest(tag~., data=res1, ntree=500,importance=T)

rank1 = fit1[[6]]
rank1 = apply(rank1,1,mean)
rank1 = rank1[order(rank1,decreasing=T)]
H3K36me3  H3K4me3  H3K9me3  H3K4me1 H3K27me3 
39.46570 35.56229 32.00639 20.56035 17.35900 
...
GM12878_AUC = 1- fit2[[3]]

GM12878_model = randomForest(tag~., data=res2, ntree=500,importance=T)

rank2 = fit2[[6]]
rank2 = apply(rank2,1,mean)
rank2 = rank2[order(rank2,decreasing=T)]

#########################
#mESC model in GM12878
########################
GM12878_pred = predict(mESC_model, res2[,-1], type="prob")
...
myauc = sum(tmp1*tmp2)
mESC_model_in_GM12878_auc = 1- myauc

#########################
#GM12878 model in mESC
########################
mESC_pred = predict(GM12878_model, res1[,-1], type="prob")
tmp = data.frame(res1[,1], mESC_pred)
res = tmp
...
myauc = sum(tmp1*tmp2)
GM12878_model_in_mESC_auc = 1- myauc

#mESC figure

data = rbind(GM12878_model_in_mESC_auc, mean(mESC_AUC))
data = as.data.frame(data)
data$target = c("GM12878_Model","mESC")
data$V1 = round(data$V1,2)
c1 <- brewer.pal(10,"RdBu")[3]

myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/GM12878/GM12878_Histone_Model_in_mESC_AUC.pdf"
...
data = rbind(mESC_model_in_GM12878_auc, mean(mESC_AUC))
data = as.data.frame(data)
data$target = c("mESC_Model","GM12878")
data$V1 = round(data$V1,2)
c1 <- brewer.pal(10,"RdBu")[3]

myoutf1 <- "/lorax/chenglab/yanding/ATAC_seq_integration/Figures/mESC/mESC_Histone_Model_in_GM12878_AUC.pdf"
pdf(myoutf1, width= 3, height= 3)
p <- position_dodge(0.1)
p1 <- ggplot(data=data, aes(x=target, y=V1, fill=c1)) + 
  geom_bar(stat="identity", color="black", position=position_dodge(),width=0.5) + scale_fill_manual(values=c(c1))+guides(fill=FALSE)+scale_y_continuous(expand = c(0,0),limits = c(0, 0.85)) + geom_text(aes(label=V1), vjust=1.6, color="white", size=3.5)
